...
struct ActivityTabView: View {
    ...
    // Cached chart data — computed once in loadData(), not per render
    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                VStack(spacing: 16) {
                    if !healthKit.isAvailable {
                        ...
                        // Today cards render immediately from prefetched healthKit
                        todayCards

                        if streak > 0 { streakCard }

                        // Charts only after data loaded
                        if hasLoaded {
                            chartRangePicker
                            stepsChart
                            caloriesChart
            ...
            .task {
                // Use prefetched data from HealthKitManager immediately
                if !hasLoaded {
                    if !healthKit.isAuthorized {
                        await healthKit.requestAuthorization()
                    }
                    if healthKit.cachedDailySteps.isEmpty {
                        // First launch — data not yet prefetched, fetch now
            ...
            .onChange(of: chartRange) {
                // Chart range change just re-slices already-cached data
    ...
    // MARK: - Data Building (synchronous — uses already-fetched data)
